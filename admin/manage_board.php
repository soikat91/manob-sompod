...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        $target_dir = "../assets/images/board/";
        if (!is_dir($target_dir)) mkdir($target_dir, 0777, true);
        $image_name = time() . '_' . basename($_FILES['image']['name']);
        $target_file = $target_dir . $image_name;
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            $image = "assets/images/board/" . $image_name;
        }
    }
    ...
    if (isset($_POST['id']) && !empty($_POST['id'])) {
        ...
        $id = (int)$_POST['id'];
        if ($image) {
            $stmt = $conn->prepare("UPDATE board_members SET name = ?, position = ?, bio = ?, image = ? WHERE id = ?");
            $stmt->bind_param("ssssi", $name, $position, $bio, $image, $id);
        } else {
            $stmt = $conn->prepare("UPDATE board_members SET name = ?, position = ?, bio = ? WHERE id = ?");
            $stmt->bind_param("sssi", $name, $position, $bio, $id);
        }
        $message = 'Team member updated successfully!';
    } else {
        // Insert
        $stmt = $conn->prepare("INSERT INTO board_members (name, position, bio, image) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $name, $position, $bio, $image);
        $message = 'Team member added successfully!';
    }
    $stmt->execute();
    $stmt->close();
    header('Location: manage_board.php?message=' . urlencode($message));
    exit();
}

// Get all team members
$board_result = $conn->query("SELECT * FROM board_members ORDER BY name");
...
<head>
    ...
    <title>Manage Board Members - Admin</title>
    ...
    <nav class="sidebar position-fixed">
        <div class="p-3">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item"><a class="nav-link" href="manage_news.php"><i class="fas fa-newspaper me-2"></i>News</a></li>
                <li class="nav-item"><a class="nav-link" href="manage_activities.php"><i class="fas fa-calendar-alt me-2"></i>Activities</a></li>
                <li class="nav-item"><a class="nav-link" href="manage_services.php"><i class="fas fa-concierge-bell me-2"></i>Services</a></li>
                <li class="nav-item"><a class="nav-link active" href="manage_board.php"><i class="fas fa-user-tie me-2"></i>Board Members</a></li>
                <li class="nav-item"><a class="nav-link" href="manage_board.php"><i class="fas fa-user-tie me-2"></i>Board Members</a></li>
                <li class="nav-item"><a class="nav-link" href="manage_gallery.php"><i class="fas fa-images me-2"></i>Gallery</a></li>
                <li class="nav-item"><a class="nav-link" href="manage_content.php"><i class="fas fa-edit me-2"></i>Content Pages</a></li>
                <li class="nav-item"><a class="nav-link" href="manage_contact.php"><i class="fas fa-address-book me-2"></i>Contact Info</a></li>
    ...
    <div class="content">
        <div class="container-fluid p-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2><i class="fas fa-user-tie me-2"></i>Manage Board Members</h2>
